...
def batch_generator(x, pad, bs, _shuffle=False,classes_num=0,HICO_IMAGE_DIR=None,HICO_POINT_MASK_DIR=None):
    ...
    while (True):
        ...
        bat_img, bat_roi_o, bat_roi_h, bat_pair_posi, bat_score_o ,bat_img_path,bat_human_bbox,bat_obj_bbox,point_arr= [], [], [], [], [] ,[],[],[],[]
        bat_label = []
        ...
        for i in range(len(x)):
            ...
            label_one_hot = to_one_hot(row['action_no']-1, classes_num)
...
def batch_multi_generator(x, pad, bs, _shuffle=False, classes_num=0, HICO_IMAGE_DIR=None, HICO_POINT_MASK_DIR=None):
    """
    returns:
    x_batch:
    y_batch:
...
if __name__ == '__main__':
    ...
    batch_multi_generator(x=train_data, pad=1, bs=batch_size,HICO_IMAGE_DIR=HICO_IMAGE_DIR,HICO_POINT_MASK_DIR=HICO_POINT_MASK_DIR,classes_num=CLASS_NUM)
