        ...
        for line in lines_list:
            try:
                if "difficult" in line:
                    ...
                    class_name, hleft, htop, hright, hbottom , oleft, otop, oright, obottom = line.split()
                    is_difficult = False
                hbbox = tuple(map(float, (hleft, htop, hright, hbottom)))
                obbox = tuple(map(float, (oleft, otop, oright, obottom)))
            except ValueError:
            ...
            if class_name in ignores:
                continue
            groundtruths[class_name][file_id].append({"hbbox":hbbox, "obbox":obbox ,"difficult":is_difficult})
    return groundtruths
...
def read_pred_files(predicted_files_list):
    ...
    for txt_file in predicted_files_list:
        ...
        for line in lines:
            try:
                class_name, confidence, hleft, htop, hright, hbottom, oleft, otop, oright, obottom= line.split()
                hbbox = tuple(map(float, (hleft, htop, hright, hbottom)))
                obbox = tuple(map(float, (oleft, otop, oright, obottom)))
                ...
                error_msg += " Received: " + line
                error(error_msg)
            predictions[class_name].append({"confidence":confidence, "file_id":file_id, "hbbox":hbbox,"obbox":obbox})
