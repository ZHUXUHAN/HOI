...
class VCOCOeval(object):
                ...
                for aid in range(self.num_actions):  # loop 26 actions
                    for j, rid in enumerate(self.roles[aid]):
                        if rid == 'agent':
                            ...
                            # if self.actions[aid] + '_' + rid not in det:
    ...
    def _do_role_eval(self, vcocodb, detections_file, ovr_thresh=0.7, eval_type='scenario_1'):
                ...
                for rid in range(len(self.roles[aid]) - 1):
                    ...
                    for j in idx:
                        ...
                        if np.all(gt_roles[jmax, :] == -1):  # if no gt role
                            ...
                            ov_role = get_overlap(gt_roles[jmax, :].reshape((1, 4)), role_boxes[j, :])#comput the role's overlap

                            # elif aid == 4:
                            #     print('4', gt_roles[jmax, :].reshape((1, 4)), role_boxes[j, :], ov_role)
        ...
        role_ap[:] = np.nan
        for aid in range(self.num_actions):
            if len(self.roles[aid]) < 2:#不算没有role的动作
                continue
            for rid in range(len(self.roles[aid]) - 1):
...
if __name__ == '__main__':
    vcocoeval = VCOCOeval('../DATA/v-coco'+'/data/vcoco/vcoco_test.json',
                          '../DATA/v-coco'+'/data/instances_vcoco_all_2014.json',
                          '../DATA/v-coco'+'/data/splits/vcoco_test.ids')
    vcocoeval._do_eval('/home/priv-lab1/workspace/zxh/HOI/vcoco_det/detection.pkl', ovr_thresh=0.5)
